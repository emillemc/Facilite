<form id="form_user" class="form-group" action="{{ route('post-editar-conta-user') }}" method="POST">
  ...
  <div class="top-6 form-group{{ $errors->has('name') ? ' has-error' : '' }}">
    ...
    <div class="col-md-9">
      <input id="name_edit" type="text" class="form-control" name="name" value="@if(isset($userName)){{old('name', $userName)}}@else{{$profName or old('name')}}@endif" maxlength="50" placeholder="Ex.: Maria José"/>
      @if ($errors->has('name'))
  ...
  <div class="top-4 form-group{{ $errors->has('email') ? ' has-error' : '' }}">
    ...
    <div class="col-md-9">
      <input id="email_edit" type="email" class="form-control" name="email" value="@if(isset($userEmail)){{old('email', $userEmail)}}@else{{$profEmail or old('email')}} @endif" maxlength="60" placeholder="Ex.: josemaria@gmail.com"/>
      @if ($errors->has('email'))
          <span class="help-block">
  ...
  @if(!isset($profCpf))
    <div class="top-4 form-group{{ $errors->has('cpf') ? ' has-error' : '' }}" id="formCpf" style="display: none">
      <label for="cpf_edit" class="col-md-2 control-label">Cpf:</label>
      <div class="col-md-9">
